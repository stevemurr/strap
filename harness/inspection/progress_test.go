...
func TestProgressRecordedPrefixAndArchive(t *testing.T) {
	...
	first, err := s.ReportWorkProgress("worker", work.ReportWorkProgressRequest{WorkTarget: work.WorkTarget{ID: w.ID, ExpectedRevision: w.Revision}, AssignedAtRevision: w.AssignedAtRevision, Position: &work.WorkPosition{Objective: "establish state"}, Findings: []work.ProgressFindingDraft{{Claim: "a limitation", Basis: work.Inferred, Limitation: "not executed"}}})
	if err != nil {
		t.Fatal(err)
	}
	firstPrefix := prefix
