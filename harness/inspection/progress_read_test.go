...
func TestProgressFragmentsPreserveEscapedRecordAndBudget(t *testing.T) {
	...
	ctx := context.Background()
	receipt, err := s.ReportWorkProgress("worker", work.ReportWorkProgressRequest{WorkTarget: work.WorkTarget{ID: w.ID, ExpectedRevision: w.Revision}, AssignedAtRevision: w.AssignedAtRevision, Position: &work.WorkPosition{Objective: strings.Repeat("\"\\\n界", 500)}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestProgressCollectionPinsPrefixAndRejectsCursorMutation(t *testing.T) {
	...
	ctx := context.Background()
	report := func() {
		result, err := s.ReportWorkProgress("worker", work.ReportWorkProgressRequest{WorkTarget: work.WorkTarget{ID: w.ID, ExpectedRevision: w.Revision}, AssignedAtRevision: w.AssignedAtRevision, Findings: []work.ProgressFindingDraft{{Claim: "concern", Basis: work.Inferred, Limitation: "unchecked"}}})
		if err != nil {
			t.Fatal(err)
		}
