...
func (r *run) finish(ctx context.Context, p Report) (Report, error) {
	...
	d, err := stage[draft](ctx, r, "synthesize", draftShape, input, true, func(d draft) error {
		...
		for _, ids := range append([][]string{d.SummaryIDs, d.RecommendationIDs}, d.Disagreements...) {
			for _, id := range ids {
				if !selected[id] {
					return errors.New("draft references unselected claim")
				}
			}
		}
		for _, id := range d.RecommendationIDs {
			if known[id].Basis != "inferred" {
				return errors.New("recommendations require qualified inferred claims")
			}
		}
	...
	for offset := 0; offset < len(selected); offset += 4 {
		...
		for _, f := range batch {
			claims = append(claims, map[string]any{"claim_id": f.ID, "claim": f.Claim, "basis": f.Basis, "limitation": f.Limitation, "evidence": f.Evidence})
		}
		...
		if err != nil {
			p.Claims = valuesFor(p.Claims, known)
			return p, err
		}
	...
	p.Claims = nil
	for _, id := range d.ClaimIDs {
		p.Claims = append(p.Claims, known[id])
	}
	...
	cov, err := stage[coverageResult](ctx, r, "coverage", coverageShape, map[string]any{"requirements": requirements, "accepted_claims": accepted}, true, func(v coverageResult) error {
		if len(v.Coverage) != len(requirements) {
			return errors.New("coverage must include every requirement")
		}
		seen := map[int]bool{}
		for _, c := range v.Coverage {
			if c.Index < 0 || c.Index >= len(requirements) || seen[c.Index] || c.Requirement != requirements[c.Index] || len(c.Reason) > 1024 || len(c.ClaimIDs) > 32 {
			...
			if c.Status != "unmet" && len(c.ClaimIDs) == 0 {
				return errors.New("coverage requires supporting claims")
			}
			for _, id := range c.ClaimIDs {
				if !supported(known[id]) {
					return fmt.Errorf("coverage references unsupported claim %s", id)
				}
			}
		}
		return nil
...
func valuesFor(claims []Claim, known map[string]Claim) []Claim {
