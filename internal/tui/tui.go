...
func (m *model) observe(event conversation.Event) {
	...
	case conversation.MessageEvent:
		...
		if msg.Progress != nil {
			if m.hasProgressReports(msg.Progress) {
				return // The report and its provenance are already in the activity.
			}
