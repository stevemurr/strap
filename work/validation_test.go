...
func TestSubmissionProgressAndReadValidation(t *testing.T) {
	...
	if _, err := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkTarget: WorkTarget{ID: "missing", ExpectedRevision: 1}}); !errors.Is(err, ErrNotFound) {
		t.Fatal(err)
	}
	if _, err := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkTarget: target(w), Steps: []StepProgress{{ID: p.Steps[0].ID}, {ID: p.Steps[0].ID}}}); !errors.Is(err, ErrInvalid) {
		t.Fatal(err)
	}
	...
	var err error
	w, err = s.reportSnapshot("impl", ReportWorkProgressRequest{WorkTarget: target(w), Steps: []StepProgress{{ID: p.Steps[0].ID, Status: ptr(Blocked), Note: ptr("dependency")}}, Position: &WorkPosition{Objective: "fixture progress", Note: *ptr("working"), Blocker: *ptr("waiting")}})
	...
	w, _ = s.reportSnapshot("impl", ReportWorkProgressRequest{WorkTarget: target(w), Position: &WorkPosition{Objective: "fixture progress", Blocker: *ptr("")}})
	w = ready(t, s, w)
	...
	if _, err := s.reportSnapshot("reviewer", ReportWorkProgressRequest{WorkTarget: target(a), Steps: []StepProgress{{ID: p.Steps[0].ID}}}); !errors.Is(err, ErrForbidden) {
		t.Fatal(err)
	}
}
