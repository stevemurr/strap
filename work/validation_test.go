...
func TestSubmissionProgressAndReadValidation(t *testing.T) {
	...
	if _, err := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkID: "missing"}); !errors.Is(err, ErrNotFound) {
		t.Fatal(err)
	}
	...
	w, _ = s.reportSnapshot("impl", ReportWorkProgressRequest{WorkID: w.ID, Position: &WorkPosition{Objective: "fixture progress", Blocker: *ptr("")}})
	w = ready(t, s, w)
	...
	if _, err := s.reportSnapshot("reviewer", ReportWorkProgressRequest{WorkID: a.ID, Steps: []StepProgress{{ID: p.Steps[0].ID}}}); !errors.Is(err, ErrForbidden) {
		t.Fatal(err)
	}
}
