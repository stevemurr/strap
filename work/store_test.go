...
func ready(t *testing.T, s *Store, w Work) Work {
	...
	w, e := s.reportSnapshot(w.Assignee, ReportWorkProgressRequest{WorkID: w.ID, Steps: changes})
	if e != nil {
		t.Fatal(e)
	}
	return w
...
func TestAuditRepairAcceptance(t *testing.T) {
	...
	w = ready(t, s, w)
	sub := submit(t, s, w)
	// Submitted work is no longer active, so reporting against it is rejected
	// by its state rather than by a revision the caller no longer has to carry.
	if _, e := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkID: w.ID, Position: &WorkPosition{Objective: "fixture progress", Note: *ptr("late")}}); !errors.Is(e, ErrState) {
		t.Fatalf("submitted update: %v", e)
	}
	current, _ := s.GetWork("root", w.ID)
	if _, e := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkID: current.ID, Position: &WorkPosition{Objective: "fixture progress", Note: *ptr("late")}}); !errors.Is(e, ErrState) {
		t.Fatalf("submitted update: %v", e)
	}
	a := review(t, s, w.ID, sub.ID)
	a, e := s.reportSnapshot("reviewer", ReportWorkProgressRequest{WorkID: a.ID, Position: &WorkPosition{Objective: "fixture progress", Blocker: *ptr("environment unavailable")}})
	if e != nil {
		t.Fatal(e)
	}
	...
	a, e = s.reportSnapshot("reviewer", ReportWorkProgressRequest{WorkID: a.ID, Position: &WorkPosition{Objective: "fixture progress", Blocker: *ptr("")}})
	if e != nil {
		t.Fatal(e)
	}
	...
	if _, e = s.reportSnapshot("impl", ReportWorkProgressRequest{WorkID: repair.ID, Steps: []StepProgress{{ID: p.Steps[1].ID, Status: ptr(InProgress)}}}); !errors.Is(e, ErrForbidden) {
...
func TestPlanScopeAndAtomicity(t *testing.T) {
	...
	if _, e := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkID: w.ID, Steps: []StepProgress{{ID: p.Steps[0].ID, Status: ptr(InProgress)}, {ID: p.Steps[2].ID, Status: ptr(ReadyForReview)}}}); !errors.Is(e, ErrForbidden) {
		t.Fatal(e)
	}
	got, _ = s.GetPlan("root", p.ID)
	...
	if _, e := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkID: w.ID, Steps: []StepProgress{{ID: p.Steps[0].ID, Status: ptr(Completed)}}}); e == nil {
...
func TestConcurrentDisjointProgress(t *testing.T) {
	...
	for _, item := range []Work{w, other} {
		...
		go func(w Work) {
			defer wg.Done()
			_, e := s.reportSnapshot(w.Assignee, ReportWorkProgressRequest{WorkID: w.ID, Steps: []StepProgress{{ID: w.Scope.StepIDs[0], Status: ptr(InProgress)}}})
			errs <- e
		}(item)
	}
	wg.Wait()
