...
func ready(t *testing.T, s *Store, w Work) Work {
	...
	w, e := s.reportSnapshot(w.Assignee, ReportWorkProgressRequest{WorkTarget: target(w), Steps: changes})
	if e != nil {
		t.Fatal(e)
	}
	return w
...
func TestAuditRepairAcceptance(t *testing.T) {
	...
	w = ready(t, s, w)
	sub := submit(t, s, w)
	if _, e := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkTarget: target(w), Position: &WorkPosition{Objective: "fixture progress", Note: *ptr("late")}}); !errors.Is(e, ErrConflict) {
		t.Fatalf("stale update: %v", e)
	}
	current, _ := s.GetWork("root", w.ID)
	if _, e := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkTarget: target(current), Position: &WorkPosition{Objective: "fixture progress", Note: *ptr("late")}}); !errors.Is(e, ErrState) {
		t.Fatalf("submitted update: %v", e)
	}
	a := review(t, s, w.ID, sub.ID)
	a, e := s.reportSnapshot("reviewer", ReportWorkProgressRequest{WorkTarget: target(a), Position: &WorkPosition{Objective: "fixture progress", Blocker: *ptr("environment unavailable")}})
	if e != nil {
		t.Fatal(e)
	}
	...
	a, e = s.reportSnapshot("reviewer", ReportWorkProgressRequest{WorkTarget: target(a), Position: &WorkPosition{Objective: "fixture progress", Blocker: *ptr("")}})
	if e != nil {
		t.Fatal(e)
	}
	...
	if _, e = s.reportSnapshot("impl", ReportWorkProgressRequest{WorkTarget: target(repair), Steps: []StepProgress{{ID: p.Steps[1].ID, Status: ptr(InProgress)}}}); !errors.Is(e, ErrForbidden) {
...
func TestPlanScopeAndAtomicity(t *testing.T) {
	...
	if _, e := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkTarget: target(w), Steps: []StepProgress{{ID: p.Steps[0].ID, Status: ptr(InProgress)}, {ID: p.Steps[2].ID, Status: ptr(ReadyForReview)}}}); !errors.Is(e, ErrForbidden) {
		t.Fatal(e)
	}
	got, _ = s.GetPlan("root", p.ID)
	...
	if _, e := s.reportSnapshot("impl", ReportWorkProgressRequest{WorkTarget: target(w), Steps: []StepProgress{{ID: p.Steps[0].ID, Status: ptr(Completed)}}}); e == nil {
...
func TestReassignmentCancellationAndSnapshots(t *testing.T) {
	...
	if _, e = s.reportSnapshot("impl", ReportWorkProgressRequest{WorkTarget: old, Position: &WorkPosition{Objective: "fixture progress", Note: *ptr("old")}}); !errors.Is(e, ErrForbidden) {
		t.Fatal(e)
	}
	if _, e = s.reportSnapshot("replacement", ReportWorkProgressRequest{WorkTarget: old, Position: &WorkPosition{Objective: "fixture progress", Note: *ptr("old")}}); !errors.Is(e, ErrConflict) {
		t.Fatal(e)
	}
...
func TestConcurrentDisjointProgress(t *testing.T) {
	...
	for _, item := range []Work{w, other} {
		...
		go func(w Work) {
			defer wg.Done()
			_, e := s.reportSnapshot(w.Assignee, ReportWorkProgressRequest{WorkTarget: target(w), Steps: []StepProgress{{ID: w.Scope.StepIDs[0], Status: ptr(InProgress)}}})
			errs <- e
		}(item)
	}
	wg.Wait()
