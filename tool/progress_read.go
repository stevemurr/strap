...
// pageBudget bounds the optional max_bytes of every progress read.
var pageBudget = []Constraint{Nullable("max_bytes", "use the default page budget"), Minimum("max_bytes", 2048), Maximum("max_bytes", 32768)}
...
func GetWorkProgress(h Handler[ProgressReadArgs]) Tool {
	...
	return compose(provider.ToolDefinition{Name: "get_work_progress", Description: "Read recorded work progress without waking workers. Select current, report, reports, finding, findings, or evidence. Follow next_cursor with only mode continue and cursor. Pages pin a snapshot; oversized records return fragments of their serialized JSON and must be reassembled before interpretation. Report and assignment revisions identify freshness. Complete evidence reads include only output retained by execution capture."},
		...
		}, append(pageBudget, Enum("mode", "current"), MinLength("work_id", 1))...),
		...
		}, append(pageBudget, Enum("mode", "reports", "findings"), Nullable("limit", "use the default page size"), MinLength("work_id", 1), Minimum("limit", 1), Maximum("limit", 100))...),
		...
		}, append(pageBudget, Enum("mode", "report"), MinLength("report_id", 1))...),
		...
		}, append(pageBudget, Enum("mode", "finding"), MinLength("finding_id", 1))...),
		...
		}, append(pageBudget, Enum("mode", "evidence"), MinLength("evidence_ref", 1))...),
